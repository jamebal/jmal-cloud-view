      ...
      <div class="article-header">
        ...
        <div class="inner">
          ...
          <div class="blog-description font-mono">
            ...
            · {{file.uploadDate}}
            <span v-if="isEditor">
              · <a :href="'/setting/website/manager-articles?operation=editor&id='+file.id" target="_blank">编辑</a>
            </span>
          </div>
        </div>
      </div>
...
export default {
    components: {SidebarNav, ArticleTop, AlBackTop, AlLoading, VditorPreview },
    data() {
      return {
        file: {},
        isLoading: true,
    ...
    methods: {
      ...
      titleOneItem(index, jqel, tagName){
      ...
      renderContents(){
        ...
        headers.each(function (index) {
          contents.push(that.titleOneItem(index, $(this), $(this).prop("tagName")))
          that.titleList.push(index)
        })
        ...
        this.renderContentsAfter(contents)
      },
      // 渲染目录后的操作
      renderContentsAfter(contents){
        ...
        $(window).on("scroll",function (event) {
          ...
          scrollAfterTimer = setTimeout(function (){
            that.$router.replace({path: that.$route.path, query: that.$route.query, hash: $('.j-titleList').find('li.active').find('a').attr('href')})
          }, 50)
        ...
        setTimeout(function (){
          if(that.$route.hash){
            $root.animate({
              scrollTop: $(decodeURIComponent(that.$route.hash)).offset().top - 75
            }, 400)
          }
        },100)
      },
